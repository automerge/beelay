...
impl CommitBundle {
    ...
    pub fn start(&self) -> Option<CommitHash> {
        self.start
    }
...
impl<T, U, V> BundleBuilder<T, U, V> {
    pub fn start(self, start: Option<CommitHash>) -> BundleBuilder<Set<Option<CommitHash>>, U, V> {
        BundleBuilder {
            start: Set(start),
            end: self.end,
...
impl BundleBuilder<Set<Option<CommitHash>>, Set<CommitHash>, Set<Vec<u8>>> {
