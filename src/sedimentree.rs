...
impl Stratum {
    ...
    pub(crate) fn parse(
        ...
        input.with_context("Stratum", |input| {
            let (input, start) = parse::maybe(input, CommitHash::parse)?;
    ...
    pub(crate) fn encode(&self, out: &mut Vec<u8>) {
        if let Some(start) = &self.meta.start {
            out.push(1);
            start.encode(out);
        } else {
            out.push(0);
    ...
    pub(crate) fn supports(&self, other: &StratumMeta) -> bool {
        ...
        if let Some(other_start) = &other.start {
            if self.checkpoints.contains(other_start) && self.checkpoints.contains(&other.end) {
                return true;
            }
            if self.checkpoints.contains(other_start) && self.meta.end == other.end {
                return true;
            }
        }
        return false;
    ...
    pub(crate) fn start(&self) -> Option<CommitHash> {
        self.meta.start
    }
...
impl StratumMeta {
    pub(crate) fn new(start: Option<CommitHash>, end: CommitHash, blob: BlobMeta) -> Self {
        Self { start, end, blob }
    }
    ...
    pub(crate) fn level(&self) -> Level {
        ...
        if let Some(start) = &self.start {
            let start_level = trailing_zeros_in_base(&start.as_bytes(), 10);
...
impl Sedimentree {
    ...
    pub(crate) fn heads(&self) -> Vec<CommitHash> {
        ...
        for stratum in minimized.strata.iter() {
            if !minimized
                .strata
                .iter()
                .any(|s| Some(s.end()) == stratum.start())
    ...
    pub(crate) fn missing_bundles(&self, doc: DocumentId) -> Vec<BundleSpec> {
        ...
        for commit_hash in dag.canonical_sequence(&self.strata) {
            tracing::trace!(commit=?commit_hash, level=%Level::from(commit_hash), "processing commit");
            let level = Level::from(commit_hash);
            for (run_level, (_start, checkpoints)) in runs_by_level.iter_mut() {
                if run_level > &level && run_level <= &TOP_STRATA_LEVEL {
                    checkpoints.push(commit_hash);
                }
            }
            if level <= TOP_STRATA_LEVEL {
                tracing::trace!("found a bundle end at level {:?}", level);
                if let Some((end, checkpoints)) = runs_by_level.remove(&level) {
                    if !self.strata.iter().any(|s| s.supports_block(commit_hash)) {
                        all_bundles.push(BundleSpec {
                            doc,
                            start: Some(commit_hash),
                            end,
                            checkpoints,
                        })
                    }
        ...
        for (_level, (end, commits)) in runs_by_level {
            all_bundles.push(BundleSpec {
                doc,
                start: None,
                end,
...
mod tests {
    ...
    use super::{Stratum, StratumMeta, TOP_STRATA_LEVEL};
    use crate::{blob::BlobMeta, parse, BundleSpec, CommitHash};
    ...
    fn stratum_supports_higher_levels() {
        ...
        impl<'a> arbitrary::Arbitrary<'a> for Scenario {
            fn arbitrary(u: &mut arbitrary::Unstructured<'a>) -> arbitrary::Result<Self> {
                let starts_at_root = bool::arbitrary(u)?;
                let start_hash = if starts_at_root {
                    None
                } else {
                    Some(hash_with_trailing_zeros(u, 10, 10)?)
                };
                ...
                let higher_start_hash: Option<CommitHash>;
                ...
                match lower_level_type {
                    ...
                    HigherLevelType::StartsAtCheckpointEndsAtEnd => {
                        let start = hash_with_trailing_zeros(u, 10, 9)?;
                        checkpoints.push(start);
                        higher_start_hash = Some(start);
    ...
    #[test]
    fn missing_bundle_on_linear_dag_with_level_2_commit() {
        ...
        while commits.len() < 20 {
            let contents = rng.gen::<[u8; 20]>().to_vec();
            let blob = BlobMeta::new(&contents);
            let hash = crate::CommitHash::from(rng.gen::<[u8; 32]>());
            if super::Level::from(&hash) <= TOP_STRATA_LEVEL {
                continue;
            }
            let commit = super::LooseCommit {
                hash,
                parents: last_hash.take().into_iter().collect(),
                blob,
            };
            commits.push(commit);
            last_hash = Some(hash);
        }
        let final_commit = super::LooseCommit {
            hash: arb_from_rand(&mut rng, |u| hash_with_trailing_zeros(u, 10, 2)),
            blob: BlobMeta::new(&[0]),
            parents: last_hash.take().into_iter().collect(),
        };
        commits.push(final_commit.clone());
        let tree = super::Sedimentree::new(vec![], commits.clone());
        let doc = crate::DocumentId::random(&mut rng);
        let mut bundles = tree.missing_bundles(doc);
        ...
        assert_eq!(
            ...
            BundleSpec {
                doc,
                start: None,
                end: final_commit.hash(),
                checkpoints: Vec::new(),
            }
        );
    }

    fn arb_from_rand<T, F, R>(rng: &mut R, f: F) -> T
    where
        F: for<'a> Fn(&'a mut arbitrary::Unstructured<'a>) -> Result<T, arbitrary::Error>,
        R: rand::Rng,
    {
        ...
        loop {
            ...
            match f(&mut unstructured) {
                Ok(value) => return value,
                Err(arbitrary::Error::NotEnoughData) => {
                    data_size *= 2;
                    continue;
                }
                Err(e) => panic!("arbitrary::Error: {:?}", e),
            }
        }
    }
}
